...
import { Wallet as WalletIcon, Receipt, ChevronRight, Sparkles, PiggyBank, Link2 } from "lucide-react";
...
export default function StudentDashboard() {
  ...
  const {
    studentName, transactions, budgets, walletBalance, weeklySpent, weeklyLimit, parentName, parentLinked, parentBalance,
  } = useOutletContext();
      ...
      {!parentLinked ? (
        <Card className="p-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Link to a parent to see your balance and request funds.
          </p>
          <button
            onClick={() => navigate("/student/profile")}
            className="mt-4 rounded-lg bg-brand-700 px-6 py-2.5 text-sm font-semibold text-white hover:bg-brand-800"
          >
            Go to Profile
          </button>
        </Card>
          ...
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
            <Card className="p-6 lg:col-span-2">
              ...
              <div className="mt-2 flex items-center gap-3">
                <span className="text-3xl font-bold font-display tabular-nums text-slate-900">{naira(parentBalance)}</span>
              </div>
              <p className="mt-1 text-xs text-slate-400">{parentName}'s wallet · available for you</p>
